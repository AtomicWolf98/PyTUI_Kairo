...
- `/key migrate [env-name]`：将当前 profile 的 inline key 迁移到环境变量，并更新 `api_key_env`。
- `/roles`：查看当前角色绑定。
...
- `/session open <id-or-index>`：只读打开某个 session，不切换到该 session。
...
- 会话搜索与只读打开：`/session search`、`/session open`。

...
def build_registry(
    ...
    *,
    skill_trust_store: SkillTrustStore = None,
) -> ToolRegistry:
    ...
    registry = ToolRegistry(skill_trust_store=skill_trust_store)
...
def build_agent(config, console=None, *, skill_trust_store: SkillTrustStore = None) -> Agent:
    workspace_context = WorkspaceContext(
        config.workspace_root,
        allow_absolute_outside=config.policy.get("workspace_path", {}).get("allow_absolute_outside", False),
    )
